#!/bin/bash
# Test restore backup database PostgreSQL gestionale - Versione Docker
# Autore: AI + Mauri
# Ultima modifica: $(date +%Y-%m-%d)
# Questo script testa il restore di un backup Docker in un database temporaneo
...
echo "[$(date)] Inizio test restore Docker backup database" | tee -a "$LOG"

# Verifica che il container PostgreSQL sia in esecuzione
if ! docker ps | grep -q "$CONTAINER_NAME"; then
  echo "[$(date)] ERRORE: Container PostgreSQL non in esecuzione" | tee -a "$LOG"
  mail -s "[ERRORE] Test Restore Docker: Container non attivo" "$MAILTO" < "$LOG"
  exit 1
fi
...
if [ -z "$LATEST_BACKUP" ]; then
  ...
  mail -s "[ERRORE] Test Restore Docker: Nessun backup trovato" "$MAILTO" < "$LOG"
  exit 1
fi

echo "[$(date)] Backup da testare: $LATEST_BACKUP" | tee -a "$LOG"

# Crea database temporaneo per il test usando Docker
...
if [ $? -ne 0 ]; then
  ...
  mail -s "[ERRORE] Test Restore Docker: Creazione database temporaneo fallita" "$MAILTO" < "$LOG"
  exit 2
fi
...
if [ $RESTORE_EXIT_CODE -ne 0 ]; then
  ...
  docker exec "$CONTAINER_NAME" dropdb "$TEST_DB_NAME" 2>/dev/null
  mail -s "[ERRORE] Test Restore Docker: Restore fallito (codice $RESTORE_EXIT_CODE)" "$MAILTO" < "$LOG"
  exit 3
fi
...
# mail -s "[SUCCESSO] Test Restore Docker: Backup verificato" "$MAILTO" <<< "Il backup $LATEST_BACKUP è stato testato con successo nel container Docker." 